...
Commands = setmetatable({
	...
	Friend = {
		...
		Commands = {"friend";"addfriend"};


		Args = {"players"};


		Description = "Sends a friend request to certain players";
	...
	UnFriend = {
		...
		Description = "Unfriends certain players";
	...
	RemoveHats = {
		...
		Function = function(plr, args)
			...
			for _, v in pairs(plr.Character and plr.Character:GetChildren() or {}) do


				if v:IsA("Accoutrement") then


					for _, joint in pairs(v:GetDescendants()) do
					...
					v.Handle.Position = v.Handle.Position - Vector3.new(0, 100000, 0)
	...
	HatPets = {
		...
		Args = {"remove_mesh (true/false)"};


		Description = "Gives you hat pets";
				...
				for _, v in pairs(character:GetChildren()) do
					...
					if v:IsA("Accoutrement") and v:findFirstChild("Handle") then
						...
						for _, joint in pairs(v:GetDescendants()) do
							...
							if joint:IsA("Weld") or joint:IsA("WeldConstraint") or joint:IsA("Attachment") then
						...
						if handle then
							...
							local bpos = service.New("BodyPosition", {
							...
							Routine(function()
								...
								while handle.Parent do
									...
									bpos.Position = character.HumanoidRootPart.Position+Vector3.new(math.random(-10,10),math.random(7,9),math.random(-10,10))


									if not bpos.Parent then


										bpos = service.New("BodyPosition", {
	...
	input.FocusLost:Connect(function(enterPressed, inputObject)
		...
		if enterPressed and inputObject and inputObject.KeyCode == Enum.KeyCode.Return then
			...
			if input.Text ~= '' and #input.Text > 1 then
				...
				service.TrackTask("Thread: Process Command", Process.Command, service.Player, input.Text, {Check = true})
